...
module.exports = function TextareaDecorator( textarea, parser ){
  ...
  var parent = this.element = document.createElement("div");
  ...
  var color = function( input, output, parser ){
    ...
    // undo newline fix from previous iteration
    ...
    for (var i = 0; i < output.childNodes.length; i++) {
      ...
      if (node !== output.lastElementChild) node.style.display = '';
    }
    if (output.lastElementChild.textContent === "\n") {
      output.lastElementChild.style.display = 'block';
    }
  };
